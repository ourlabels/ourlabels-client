...
import { Container, Segment, Card, Header, Icon, Button, Image } from 'semantic-ui-react';
...
class ProjectsCardsComponent extends React.Component {
      ...
      <Container>
        ...
        {current && project.joined && (
          <Icon
            ...
            data-iscapture="true"
            fitted
            onClick={() => {this.props.leaveProject(project.id);}}
          />
        )}
        {!current && !project.joined && (type === 'owned' || type === 'allowed' || project.publicType) &&
              <Icon 
                name="linkify"  
                size="large"
                data-for={'join'}
                ...
                data-iscapture="true"
                fitted
                onClick={() => {this.props.joinProject(project.id);}}
              />
        }
        {!current && project.joined && (type === 'owned' || type === 'allowed' || project.publicType) &&
              <Icon 
                ...
                data-iscapture="true"
                fitted
                onClick={() => {this.props.changeProject(project.id);}}
              />
        }
        
        {current && (type === 'owned' || project.publicType) &&
            <div
              className="icon-div"
              data-for={'json'}
              data-tip="download annotations as JSON"
              data-iscapture="true"
              onClick={() => {this.props.getProjectAnnotations(project.id, 'json');}}
            >
            J
            </div>
        }
        {current && (type === 'owned' || project.publicType) &&
            <div
              ...
              data-iscapture="true"
              onClick={() => {this.props.getProjectAnnotations(project.id, 'xml');}}
            >
              X
            </div>
        }
  ...
  render() {
    const {
      type, items, filter, projectTypes,
    } = this.props;
          ...
          <Card.Group>
            {
              itemsToDisplay.map((item, index) => {
                      ...
                      {featured && (
                        <Container>
                          <Icon 
                            ...
                            data-iscapture="true"
                          />
                          <ReactTooltip id="featured" place="bottom" type="dark" />
                        </Container>
                      )}
                      {currentProject && (
                        <Container>
                ...
                const titleHeader = (
                  <Card.Description>
                    <Button onClick={this.showMoreInfo(index)}><Header as="h3">{title}</Header></Button><br/>
                    {description}
                  </Card.Description>);
